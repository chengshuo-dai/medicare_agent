...
class DiagnosisReport(BaseModel):
    ...
    disclaimer: str = Field(
        default="This report is AI-generated for reference only and does not replace professional medical diagnosis.",
...
EXAMPLES:
- Input: "Chief complaint: headache HPI: frontal headache fever 38.5C"
  Output: frontal headache fever differential diagnosis medical

- Input: "Chief complaint: chest tightness shortness of breath HPI: worse with exertion history of hypertension"
  Output: chest tightness dyspnea hypertension cardiovascular medical

- Input: "Chief complaint: abdominal pain HPI: severe right lower quadrant pain fever nausea"
  Output: right lower quadrant pain fever nausea acute abdomen medical
...
OUTPUT (search query only):"""
        ...
        if not query or len(query) < 5:
            ...
            query = f"{chief} differential diagnosis medical"
    ...
    async def analyze(
        ...
        user_msg = f"## Patient Chief Complaint & Interview Information\n{symptoms}"
        if patient_history:
            user_msg += f"\n\n## Medical History\n{patient_history}"
        if test_results:
            user_msg += f"\n\n## Test Results\n{test_results}"
        if knowledge_context:
            user_msg += (
                f"\n\n## Medical Knowledge Base Search Results (obtained via RAG + SearXNG real-time search)\n"
                f"{knowledge_context}\n\n"
                f"Based on the above search results and patient information, generate a diagnosis, "
                f"citing these sources in your reasoning."
            )
        if patient_id:
            user_msg += f"\n\nPatient ID: {patient_id} (use query_patient_history tool to retrieve past medical records)"
        ...
        async with async_session_maker() as db:
            ...
            for _round in range(5):
                if _round == 4:
                    ...
                    messages.append({
                        "role": "user",
                        "content": "All tools have been called. Now, based on ALL the above information, output the structured JSON diagnosis report directly. Do NOT call any more tools."
                    })
            ...
            if structured is None:
                try:
                    force_messages = messages + [{
                        "role": "user",
                        "content": (
                            "Based on the above information, generate a structured diagnosis report."
                            "Must include: primary_diagnosis, differential_diagnoses (2-5 items), confidence, severity, "
                            "key_findings, recommended_tests, recommended_actions, red_flags, knowledge_sources."
                            "Each diagnosis (primary_diagnosis and each item in differential_diagnoses) MUST include the correct ICD-11 code (icd11_code),"
                            " e.g. J05.0 (acute laryngitis), J00 (acute nasopharyngitis). Output must be valid JSON format."
                        ),
                    }]
            ...
            if structured is None:
                try:
                    force_messages = messages + [{
                        "role": "user",
                        "content": (
                            "Based on ALL the above information, directly output a JSON-format diagnosis report."
                    ...
                    resp = await llm.chat(
                        messages=force_messages,
                        system_prompt="You are a medical diagnosis AI. Output ONLY a JSON-format diagnosis report.",
                        max_tokens=4096,
                    )
            ...
            if structured is None:
                structured = DiagnosisReport(
                    primary_diagnosis="AI diagnosis generation failed",
                    differential_diagnoses=[],
                    confidence="low",
                    severity="unknown",
                    key_findings=["System temporarily unable to generate diagnosis report. The LLM service may be busy or returning unexpected responses."],
                    recommended_tests=[],
                    recommended_actions=["Please try again later, or contact administrator to check LLM service status."],
                    red_flags=[],
                    knowledge_sources=[],
                )
...
"""
    ...
    async def plan(
        ...
        async with async_session_maker() as db:
            ...
            user_msg = f"Diagnosis: {diag_text}"
            if patient_profile:
                user_msg += f"\nPatient info: {json.dumps(patient_profile, ensure_ascii=False)}"
            if constraints:
                user_msg += f"\n约束: {', '.join(constraints)}"
...
"""
    ...
    async def check(
        ...
        async with async_session_maker() as db:
            ...
            user_msg = f"Patient latest feedback: {patient_updates}"
            if baseline_status:
    ...
    async def route(
        ...
        if intent == "diagnosis":
            ...
            mon_result = await self.monitoring.check(
                patient_updates=f"初次Diagnosis: {user_input}",
