...
TRACK1_SYSTEM_PROMPT = """You are a medical history-taking specialist. Based on the patient's chief complaint and collected information, generate interview questions following clinical practice standards.

Responsibilities:
- Cover unasked clinical dimensions: HPI (onset/symptom characteristics/accompanying symptoms/progression/treatment history), past medical history, personal history (including general status), family history, medication history
- Special populations: pediatrics (feeding/development/vaccination), women (menstrual/obstetric) — only when applicable
- Use conversational, patient-friendly language. Always use choice or multi_choice format. For screening questions (chronic disease history, allergies, etc.), append "None of the above" as a final option
- Generate 1-2 questions per round

Respond ONLY with JSON (wrapped in ```json```)."""

TRACK1_DECISION_SCHEMA = """Return JSON (example — generate real clinical options):
{
  "action": "ask",
  "basic_module": [
    {"question_id":"hpi_quality","question":"How would you describe the pain in your neck and shoulder area?","type":"multi_choice","options":["Aching","Stabbing","Swelling sensation","Numbness","Stiffness"],"hint":"Select all that apply","allow_skip":true,"phase":"HPI - Symptom Character","reason":"Clarify pain quality"},
    {"question_id":"hpi_severity","question":"How severe is the pain?","type":"choice","options":["Mild — does not affect activity","Moderate — limited neck movement","Severe — completely unable to move"],"hint":"","allow_skip":false,"phase":"HPI - Severity","reason":"Assess severity level"}
  ],
  "differential_diagnoses": [{"diagnosis":"Cervical myofascial pain","confidence":"medium","key_features":["Worse in morning","Improves with movement"],"reason":"Matches chief complaint"}],
  "red_flags": [],
  "covered_dimensions": ["hpi_quality"],
  "reasoning": "Inferred from symptom characteristics"
}
IMPORTANT: multi_choice options must be specific answer choices (≥2), never use placeholders. red_flags should be an array of strings (e.g. ["Frequent vomiting, unable to eat"]), NOT an array of objects."""
...
TRACK2_SYSTEM_PROMPT = """You are a search-enhanced clinical interview specialist. Based on SearXNG + RAG search results, generate targeted follow-up questions.

Responsibilities:
- Analyze key clinical clues from search results
- Design confirmation questions targeting confirmed/missing differential diagnosis features
- Identify evidence gaps from search → create new questions to fill them
- Generate 1-2 questions
- choice and multi_choice types must include at least 2 specific options; screening questions should append "None of the above"
- Never return empty options arrays

Respond ONLY with JSON (wrapped in ```json```)."""

TRACK2_DECISION_SCHEMA = """Return JSON (example — generate real clinical options):
{
  "advanced_module": [
    {"question_id":"adv_dehydration","question":"Do you have any of the following signs of dehydration?","type":"multi_choice","options":["No tears when crying","Decreased urination","Sunken eyes","Dry lips","Lethargy","None of the above"],"hint":"Select all that apply","allow_skip":true,"reason":"Assess dehydration severity"},
    {"question_id":"adv_diet","question":"Has your diet changed recently?","type":"choice","options":["Eating normally","Reduced intake","Refusing food","Only drinking water"],"hint":"","allow_skip":true,"reason":"Evaluate dietary intake"}
  ]
}
IMPORTANT: choice type must include options (≥2 specific choices), no empty arrays. multi_choice options should have "None of the above" as the last item."""
...
class Track1Agent:
    ...
    def _build_prompt(self, state: InterviewState, patient_history: str | None) -> str:
        lines = [f"## Chief Complaint\n{state.chief_complaint or 'Unknown'}"]
        if patient_history:
        ...
        lines.append("\n## Instructions\nOnly generate basic_module, 1-2 questions per round. Prioritize covering \"Unasked Dimensions\".")
...
class Track2Agent:
    ...
    def _build_prompt(self, state: InterviewState, search_results: str, diffs: list[DifferentialHypothesis]) -> str:
        ...
        if diffs:
            ...
            for d in diffs[:5]:
                lines.append(f"- {d.diagnosis} (confidence: {d.confidence}) {d.key_features}")
        if state.collected_info:
            lines.append("\n## Collected Information")
            for k, v in list(state.collected_info.items())[-8:]:
                if not k.startswith("__") and v:
                    lines.append(f"- {k}: {str(v)[:100]}")
        lines.append("\n## Instructions\nOnly generate advanced_module, 0-2 questions. Convert evidence gaps found in search results into new questions.")
    ...
    def _to_templates(module: list, state: InterviewState) -> list[QuestionTemplate]:
        ...
        for m in module:
            ...
            result.append(QuestionTemplate(
                ...
                phase=m.get("phase", "Search Supplement"),
                colloquial_phase=m.get("phase", "Search Supplement"),
            ))
        return result
...
class InterviewOrchestrator:
        ...
        for q in deduped:
            ...
            if q.type in ("multi_choice", "choice") and (not q.options or len(q.options) < 2):
                ...
                if opts:
                    ...
                    filtered_out.add(q.question_id)
                    continue
            if q.type == "multi_choice" and q.options and "None of the above" not in q.options:
                q.options = list(q.options) + ["None of the above"]
        if filtered_out:
    ...
    async def _complete_options(self, q: QuestionTemplate) -> list[str]:
        try:
            r = await self.track1.llm.chat(
                messages=[{"role": "user", "content": f'Question: {q.question}\nFor this question, list 3-6 specific options. If the patient may answer "none", add a "None of the above" option at the end. Return ONLY a JSON array.'}],
                system_prompt="You are a clinical option generation assistant. Return ONLY a JSON array.", max_tokens=256)
    ...
    async def _semantic_dedup(
        ...
        prompt = (
            "Determine whether each candidate question is semantically duplicate with any previously asked question"
            " (asking the same thing, just phrased differently).\n\n"
            ...
            + "\n\nReturn JSON: {\"keep\": [indices], \"drop\": [indices], \"reasons\": {\"index\": \"why duplicate or why keep\"}}"
            + "\nIndices start at 1. keep = genuinely new questions to retain, drop = semantically duplicate questions to discard."
        )
        try:
            response = await self.track1.llm.chat(
                messages=[{"role": "user", "content": prompt}],
                system_prompt="You are a medical AI deduplication assistant. Return ONLY JSON, no other output.",
                max_tokens=256,
            )
