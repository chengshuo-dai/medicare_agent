...
const CATEGORIES: Record<string, { label: string; color: string }> = {
  general: { label: '⚙️ General', color: '#64748B' },
  auth: { label: '🔐 Registration & Auth', color: '#3B82F6' },
  diagnosis: { label: '🏥 Medical Diagnosis', color: '#10B981' },
  agent: { label: '🤖 Agent Config', color: '#8B5CF6' },
  notification: { label: '📧 Notifications', color: '#F59E0B' },
  security: { label: '🛡️ Security', color: '#EF4444' },
  external_search: { label: '🔍 External Search', color: '#06B6D4' },
};
  ...
  if (value_type === 'boolean') {
    return (
      <FormControlLabel
        ...
        label={parseBoolean(value) ? 'Enabled' : 'Disabled'}
      />
    );
  }
...
export default function SystemSettingsPage() {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPwdSuccess('Admin password changed successfully. Please login with the new password.');
      setPwdOld('');
      setPwdNew('');
      setPwdConfirm('');
  ...
  const handleSaveCategory = async () => {
    ...
    try {
      ...
      setSuccess(`${getCategoryLabel(activeTab)} settings saved`);
      ...
      <PageHeader
        ...
        actions={(
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            >
              {saving ? <CircularProgress size={16} /> : 'Save All'}
            </Button>
      ...
      <Card variant="outlined" sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            For system security, it is recommended to change the admin password regularly. Password must be at least 8 characters.
          </Typography>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <FormControl size="small" fullWidth>
              ...
              >
                ...
                <MenuItem value="boolean">Toggle (boolean)</MenuItem>
            ...
            <FormControlLabel
              ...
              label="Sensitive setting (hidden display)"
            />
          </Box>
        </DialogContent>
        <DialogActions>
