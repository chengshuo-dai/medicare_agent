        ...
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 1.5, px: 2 }}>
          {quickReplies.map((reply) => (
            <Chip key={reply} label={reply} size="small" clickable onClick={() => onQuickReply?.(reply)}
              sx={{ bgcolor: '#F0F3EE', border: '1px solid #D9D6CE', color: 'text.primary', '&:hover': { bgcolor: '#D9D6CE', borderColor: 'primary.main' }, fontSize: 13, height: 28 }} />
          ))}
        </Box>
      )}

      <Paper elevation={2} sx={{ display: 'flex', alignItems: 'flex-end', gap: 1, p: 1.5, borderRadius: 3, border: '1px solid #D9D6CE', bgcolor: 'background.paper' }}>
        <input
          ref={fileInputRef}
          type="file"
          hidden
        ...
        <TextField
          ...
          multiline
          maxRows={4}
              placeholder={disabled ? 'Please wait...' : (placeholder || 'Describe your symptoms...')}
          value={text}
        ...
        <Button variant="contained" size="small" onClick={handleSend} disabled={disabled || !text.trim()}
          sx={{ minWidth: 40, width: 40, height: 40, borderRadius: '50%', p: 0, bgcolor: 'primary.main', '&:hover': { bgcolor: 'primary.dark' }, '&.Mui-disabled': { bgcolor: '#D9D6CE', color: 'text.secondary' } }}>
