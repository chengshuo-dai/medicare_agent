      ...
      <Box sx={{
        ...
        {!isInterviewQ && (
          <Paper elevation={0} sx={{
            ...
            background: isAgent ? '#D9D6CE' : 'background.paper',
            ...
            <Box sx={{ color: 'text.primary', wordBreak: 'break-word', lineHeight: 1.6, whiteSpace: 'pre-wrap', '& > *:first-of-type': { mt: 0 }, '& > *:last-of-type': { mb: 0 } }}>
              <ReactMarkdown
                ...
                components={{
                  ...
                  h1: ({ children }) => <Typography variant="h6" sx={{ fontWeight: 700, color: sageText, mt: 1.5, mb: 0.75 }}>{children}</Typography>,
                  h2: ({ children }) => <Typography variant="subtitle1" sx={{ fontWeight: 700, color: sageText, mt: 1.25, mb: 0.5 }}>{children}</Typography>,
                  h3: ({ children }) => <Typography variant="subtitle2" sx={{ fontWeight: 700, color: sageText, mt: 1, mb: 0.5 }}>{children}</Typography>,
                  strong: ({ children }) => <Box component="strong" sx={{ fontWeight: 700, color: sagePrimary }}>{children}</Box>,
                  em: ({ children }) => <Box component="em" sx={{ fontStyle: 'italic', color: '#6B7D6B' }}>{children}</Box>,
                  ...
                  code: ({ children, className }) => {
                    ...
                    return (
                      <Box component="code" sx={{
                        ...
                        bgcolor: isInline ? 'rgba(125,155,118,0.15)' : '#1E293B',
                        color: isInline ? sageText : '#D9D6CE',
                  ...
                  pre: ({ children }) => (
                    <Box component="pre" sx={{ bgcolor: '#1E293B', p: 1.5, borderRadius: 1.5, overflowX: 'auto', mb: 1 }}>{children}</Box>
                  ),
                  blockquote: ({ children }) => (
                    <Box component="blockquote" sx={{ borderLeft: `3px solid ${sagePrimary}`, pl: 1.5, ml: 0, color: '#6B7D6B', fontStyle: 'italic', mb: 1 }}>{children}</Box>
                  ),
                  a: ({ children, href }) => (
                    <Box component="a" href={href} target="_blank" rel="noopener noreferrer" sx={{ color: sagePrimary, textDecoration: 'underline', '&:hover': { color: '#5C7A55' } }}>{children}</Box>
                  ),
                  ...
                  thead: ({ children }) => <Box component="thead" sx={{ bgcolor: 'rgba(125,155,118,0.15)' }}>{children}</Box>,
                  th: ({ children }) => (
                    <Box component="th" sx={{ border: '1px solid #F5E6D3', p: 0.75, textAlign: 'left', fontWeight: 700, color: sageText }}>{children}</Box>
                  ),
                  td: ({ children }) => (
                    <Box component="td" sx={{ border: '1px solid #F5E6D3', p: 0.75, color: 'text.primary' }}>{children}</Box>
                  ),
                }}
              >
        ...
        {isInterviewQ && (
          <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, fontStyle: 'italic' }}>
            📋 Interview cards moved to pending panel below
          </Typography>
        )}
        ...
        {isAgent && message.toolCalls && message.toolCalls.length > 0 && (
          <Box sx={{ mt: 1 }}>
            {message.toolCalls.map((tc, i) => (
              <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1, bgcolor: '#F0F3EE', border: '1px dashed #F5E6D3', borderRadius: 2, px: 1.5, py: 0.75, mb: 0.5 }}>
                <Typography variant="caption" color="text.secondary">🔧 {tc.tool}</Typography>
                {tc.result !== undefined && <Typography variant="caption" color="success.main">✅ Completed</Typography>}
              </Box>
            ))}
          </Box>
        )}
