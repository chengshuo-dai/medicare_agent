    ...
    <Fade in={visible} timeout={400}>
      ...
      >
        ...
        <Box sx={{ flex: 1, px: 2.5, py: 2, maxWidth: 720, mx: 'auto', width: '100%' }}>
          {/* Primary Diagnosis */}
          <Section icon="🏥" title="Preliminary Diagnosis">
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1, flexWrap: 'wrap' }}>
              ...
              <Chip
                label={report.confidence === 'high' ? 'High' : report.confidence === 'medium' ? 'Medium' : 'Low'}
                size="small"
              ...
              {report.severity && (
                <Chip
                  label={report.severity === 'mild' ? 'Mild' : report.severity === 'moderate' ? 'Moderate' : report.severity === 'severe' ? 'Severe' : report.severity}
                  size="small"
                  variant="outlined"
                  sx={{ height: 22, fontSize: 12 }}
                />
          ...
          {report.differential_diagnoses && report.differential_diagnoses.length > 0 && (
            <>
              <Section icon="🔍" title="Differential Diagnosis">
                <Stack spacing={0.5}>
          ...
          {report.red_flags && report.red_flags.length > 0 && (
            <Section icon="🚨" title="Red Flags (Seek Immediate Care)">
          ...
          <Box sx={{ mt: 3, pt: 2, borderTop: '1px solid', borderColor: 'divider' }}>
            <Typography variant="caption" color="text.secondary" sx={{ lineHeight: 1.6 }}>
              {report.disclaimer || 'This report is AI-generated and for reference only. It does not replace professional medical diagnosis. Patients should seek timely medical care and receive comprehensive evaluation and diagnosis from a doctor.'}
            </Typography>
          </Box>
        </Box>
      </Box>
