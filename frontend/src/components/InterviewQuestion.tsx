...
const PHASE_COLORS: Record<string, { bg: string; border: string; icon: string }> = {
  ...
  'Supplement': { bg: '#F5F5F5', border: '#BDBDBD', icon: '📝' },
};

function getPhaseStyle(phase?: string) {
  if (!phase) return { bg: '#F0F3EE', border: '#D9D6CE', icon: '💬' };
  return PHASE_COLORS[phase] || { bg: '#F0F3EE', border: '#D9D6CE', icon: '💬' };
}
    ...
    <Fade in={true} timeout={400}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 1 }}>
          ...
          >
            {question.colloquial_phase || 'Consultation'}
          </Typography>
          <Box sx={{ flex: 1 }} />
          <MedicalInformationIcon sx={{ fontSize: 16, color: phaseStyle.border }} />
        </Box>

        <Typography variant="subtitle2" sx={{ fontWeight: 600, color: '#2C3E2D', mb: 1.5, lineHeight: 1.5, wordBreak: 'break-word', overflowWrap: 'break-word', fontSize: '0.95rem' }}>
          {question.question}
        </Typography>
          ...
          <Box>
              ...
              {question.options.map((option) => (
                <Chip
                  ...
                  sx={{
                    ...
                    color: selectedOptions.has(option) ? '#FFFFFF' : '#2C3E2D',
                    border: `1.5px solid ${phaseStyle.border}`,
            ...
            <Button variant="contained" size="small" onClick={handleMultiSubmit}
              ...
              sx={{ borderRadius: 2, bgcolor: phaseStyle.border,
                ...
                '&.Mui-disabled': { bgcolor: '#D9D6CE', color: '#6B7D6B' },
